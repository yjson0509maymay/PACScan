...
def xai_report(
    ...
    if model_connected:
        ...
        status_class = "model-demo"
        status_text = "모델 연결 전 · 시연용 결과"
        status_note = "현재 확률과 판독문은 임상 진단 결과가 아닙니다."
...
.viewer-guide{margin:2px 0 8px;padding:8px 11px;border-left:3px solid #278fff;background:#07182a;color:#a9bdd2;font-size:11px}[data-testid="stSlider"]{padding:6px 10px 2px;border:1px solid #193550;border-radius:7px;background:#07172a}[data-testid="stSlider"] [data-testid="stWidgetLabel"] p{font-size:15px!important;font-weight:600!important;color:#dcecff!important}
...
@media(max-width:650px){.topbar{min-height:112px;padding:8px 12px;gap:12px}.meta{display:none}.brand{font-size:17px;gap:9px}.brand img{width:92px;height:92px;max-height:none}.topnav{gap:13px;font-size:11px;min-width:0}.topnav a{text-align:center}.stepper{grid-template-columns:1fr 1fr}.file-grid,.qc-grid,.patient-profile{grid-template-columns:1fr 1fr}.report>header{min-height:112px;padding:8px 12px;font-size:17px;gap:12px}.report header img{width:92px;height:92px;max-height:none}.report main{padding:9px}.compare img{height:230px}.rbar{grid-template-columns:78px 1fr 40px}.report footer{gap:10px}}
...
with center:
    ...
    if not file_items and not history_result_active:
        ...
        model_connected = st.session_state.get("model_connected", False)
        ...
        ai_tab_label = "AI 분석" if model_connected else "AI 분석 (시연용)"
        view_options = ["원본 MRI", "전처리 결과", ai_tab_label, "XAI 보고서"]
        ...
        if model_connected and model_result:
            result = Result(
                normal=model_result["normal"], prodromal=model_result["prodromal"], pd=model_result["pd"],
                finding=model_result["finding"], rationale=model_result["rationale"],
            )
        else:
            result = Result()
        if view == "원본 MRI":
            ...
            if model_connected and model_result:
                ...
                # 파킨슨병 3개 확률 다 보여주도록 변경 - 데모 화면(probability())과
                # 동일한 다크테마 바 스타일 재사용(report_bar는 흰 배경 XAI 보고서용이라
                # 이 어두운 패널에서는 대비가 안 맞음).
                st.markdown(
                    f'<section class="panel"><div class="head"><span>▤</span>AI 진단 확률 요약</div><div class="pad">'
                    f"{probability('정상', model_result['normal'], '#1a9d79')}"
                    f"{probability('전구기', model_result['prodromal'], '#e5a315')}"
                    f"{probability('파킨슨병', model_result['pd'], '#ff334b')}"
                    f'</div></section>',
                    unsafe_allow_html=True,
                )
