...
def xai_report(
    ...
    return f'''<section class="report">
      ...
      <div class="model-info">
        ...
        <div><b>예정 모델</b><span>3D-CNN + 3D-ResNet<small>다중 시점 어텐션 · M3d-CAM 설명 시각화</small></span></div>
...
:root{--bg:#030b16;--panel:#0a182a;--line:#1d324a;--blue:#218cff;--text:#edf5ff;--muted:#8195ac}*{box-sizing:border-box;font-family:Inter,'Noto Sans KR',sans-serif;word-break:keep-all;overflow-wrap:break-word}.stApp{background:radial-gradient(circle at 45% -10%,#10294b,#030b16 42%);color:var(--text)}[data-testid="stHeader"],#MainMenu,footer,[data-testid="stToolbar"]{display:none!important}.block-container{max-width:1600px;padding:.6rem .8rem 2rem}[data-testid="stHorizontalBlock"],[data-testid="stColumn"]{min-width:0!important}
...
[data-testid="stFileUploader"]{width:100%;min-width:0;background:#071729;border:1px dashed #3779ba;border-radius:7px;padding:3px;color:#eaf4ff!important;overflow:hidden}[data-testid="stFileUploader"] section{width:100%;min-width:0;padding:8px!important;background:#071729!important;display:flex!important;flex-direction:column!important;align-items:stretch!important;gap:7px!important}[data-testid="stFileUploader"] *{color:#dcecff!important}[data-testid="stFileUploader"] button{width:100%!important;min-width:0!important;max-width:100%!important;margin:0!important;padding:8px 5px!important;background:#12345b!important;border:1px solid #357abb!important;color:#fff!important;overflow:hidden!important}[data-testid="stFileUploader"] button p{width:100%;font-size:0!important;white-space:normal!important}[data-testid="stFileUploader"] button p::after{content:"DICOM 폴더 선택";font-size:11px!important;line-height:1.3;color:#fff!important}[data-testid="stFileUploader"] small,[data-testid="stFileUploaderDropzoneInstructions"] small,[data-testid="stFileUploader"] [data-testid="stFileUploaderFileErrorMessage"]{display:none!important}[data-testid="stFileUploaderDropzoneInstructions"]{width:100%;min-width:0;text-align:center}[data-testid="stFileUploaderDropzoneInstructions"] span{color:#dcecff!important;opacity:1!important;font-size:10px!important;white-space:normal!important;word-break:keep-all!important;overflow-wrap:break-word!important}[data-testid="stWidgetLabel"],[data-testid="stWidgetLabel"] p{color:#dcecff!important;opacity:1!important;white-space:normal!important;word-break:keep-all!important;overflow-wrap:break-word!important}.stButton button,.stDownloadButton button{width:100%;background:#0d315d;border:1px solid #2478c8;color:#eaf5ff!important}.stButton button p,.stDownloadButton button p{color:#eaf5ff!important}.stButton button[kind="primary"]{background:linear-gradient(90deg,#1265d0,#218cff);font-weight:700}[data-testid="stSegmentedControl"]{background:#061426;border:1px solid #1c3856;border-radius:8px;padding:4px}[data-testid="stSegmentedControl"] label,[data-testid="stSegmentedControl"] p{color:#d9e9fa!important;opacity:1!important}[data-testid="stAlert"] *{color:#dcecff!important}[data-testid="stProgress"] p,[data-testid="stStatusWidget"] *{color:#dcecff!important}.status{text-align:right;color:#34d8ad;font-size:9px;margin:8px}.status.idle{color:#8195ac}.status.ready{color:#58b0ff}.status.demo{color:#ffd76a}.status.error{color:#ff7783}
...
@media(max-width:650px){.topbar{min-height:64px;padding:8px 12px;gap:14px}.meta{display:none}.brand{font-size:17px;gap:9px}.brand img{width:110px;max-height:45px}.topnav{gap:13px;font-size:11px;min-width:0}.topnav a{text-align:center}.stepper{grid-template-columns:1fr 1fr}.file-grid,.qc-grid,.patient-profile{grid-template-columns:1fr 1fr}.report>header{min-height:72px;padding:12px;font-size:17px;gap:12px}.report header img{width:115px;max-height:48px}.report main{padding:9px}.compare img{height:230px}.rbar{grid-template-columns:78px 1fr 40px}.report footer{gap:10px}}
...
with center:
    ...
    if not file_items and not history_result_active:
        ...
        if view == "원본 MRI":
            ...
            st.markdown(f'<div class="qc-grid"><div class="qc ok"><small>NIfTI 검증</small><b>✓ 통과</b></div><div class="qc ok"><small>영상 방향</small><b>✓ {prep["orientation"]}</b></div><div class="qc ok"><small>밝기 정규화</small><b>✓ Min-Max</b></div><div class="qc ok"><small>출력 크기</small><b>✓ {prep["final_shape"]}</b></div></div>', unsafe_allow_html=True)
    ...
    if st.session_state.pipeline_done:
        ...
        if active_view == "원본 MRI":
            source_info = (
                ...
                f'<b>복셀 간격 / 방향</b><br>{prep["spacing"]} · {prep["orientation"]}</div>'
            )
            ...
            st.write("")
            panel("AI 모델 상태", '<div class="reason"><b>현재 상태</b><br>모델 연결 전 시연용 결과<br><br><b>예정 모델</b><br>3D-CNN + 3D-ResNet<br>다중 시점 어텐션</div>', "▣")
        else:
            report_status = (
